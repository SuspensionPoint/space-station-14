...
using Content.Shared.MapEditor;
...
public sealed partial class MapEditorScreen : UIScreen
{
    ...
    /// </summary>
    public event Action<PaintTarget>? OnTileSelected;
    ...
    private string _selectedProtoId = string.Empty;
    ...
    private void BuildEntityInfoPanel()
    {
        ...
        var rotCCW = new Button
            { Text = "↶ CCW", MinWidth = 50, MinHeight = 24, ToolTip = "Rotate counter-clockwise (Shift+R)" };
    ...
    public void RefreshStructureList()
    {
        ...
        {
            var editorCategory = category.Value;

            var categoryHolder = new StructureCategoryEntry(editorCategory.CategoryName);

            // Okay, now let's add the entries.
            foreach (var proto in editorCategory.EntityPrototypes)
            {
                var entEntry = new StructureCategoryItemEntry(proto.Name);
                ...
                if (icon != null)
                {
                    ...
                    entEntry.RegisterTexture(displayTexture);
                }

                var capturedProtoId = proto.ID;
                var capturedName = proto.Name;
                var capturedIcon = icon;
                entEntry.Button.OnPressed += _ =>
                {
                    var entry = new PaintTarget()
                    {
                        EntityPrototype = capturedProtoId,
                        Type = PaintTargetType.Entity
                    };
                    // SetSelectedTileButton(button, capturedProtoId);
                    OnTileSelected?.Invoke(entry);
                    UpdatePreview(capturedIcon, capturedName);
                };

                // Re-apply selection highlight if this tile was previously selected.
                if (capturedProtoId == _selectedProtoId)
                {
                    ApplyTileButtonHighlight(entEntry, selected: true);
                    _selectedTileButton = entEntry.Button;
                }

                categoryHolder.EntriesHolder.AddChild(entEntry);
            }

            StructureListContainer.AddChild(categoryHolder);
        }
    ...
    public void BuildProtoIndex()
    {
        EditorCategories.Clear();

        foreach (var categoryProto in _prototypeManager.EnumeratePrototypes<EditorCategoryDefinition>())
        {
            var category = new EditorCategory(categoryProto.Name);

            foreach (var entry in categoryProto.Entries)
            {
                if (_prototypeManager.TryIndex<EntityPrototype>(entry, out var entityProto))
                    category.EntityPrototypes.Add(entityProto);
            }

            EditorCategories[categoryProto.ID] = category;
        }
    }
